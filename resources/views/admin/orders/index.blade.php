...
<div class="font-sans">
    ...
    <div class="mb-4 flex flex-wrap gap-2">
        @php
            $tabStatuses = ['all', 'Pending', 'Processing', 'Shipped', 'Delivered', 'Completed', 'Cancelled', 'Failed'];
        @endphp
        @foreach($tabStatuses as $tab)
            <a
                href="{{ route('admin.orders.index', ['status' => $tab, 'search' => $search ?: null]) }}"
                class="rounded-full px-4 py-2 text-xs font-bold {{ $status === $tab ? 'bg-[#F54E4E] text-white' : 'bg-white text-gray-700 border border-gray-200' }}"
            >
                {{ $tab === 'all' ? 'All' : $tab }}
            </a>
        @endforeach
                                ...
                                <div class="flex items-center justify-end gap-2 opacity-100 transition-opacity">
                                    ...
                                    @if(
                                        ...
                                        <form action="{{ route('admin.orders.event', $order) }}" method="POST">
                                            @csrf
                                            @method('PATCH')
                                            <input type="hidden" name="event" value="approve_payment" />
                                            <button type="submit" class="h-[34px] rounded-lg bg-green-500 hover:bg-green-600 transition-colors px-3 text-xs font-bold text-white shadow-sm">
                                                Approve Pay
                                            </button>
                                        </form>
                                        <form action="{{ route('admin.orders.event', $order) }}" method="POST">
                                            @csrf
                                            @method('PATCH')
                                            <input type="hidden" name="event" value="reject_payment" />
                                            <button type="submit" class="h-[34px] rounded-lg bg-red-500 hover:bg-red-600 transition-colors px-3 text-xs font-bold text-white shadow-sm">
                                                Reject Pay
                                            </button>
                                        </form>
                                    @endif
                                    ...
                                    @elseif(!in_array($order->order_status, ['Completed', 'Cancelled', 'Failed'], true))
                                        <form action="{{ route('admin.orders.event', $order) }}" method="POST">
                                            @csrf
                                            @method('PATCH')
                                            <input type="hidden" name="event" value="delivery_preparing" />
                                    ...
                                    @if(in_array($order->order_status, ['Pending', 'Processing'], true))
                                        <form action="{{ route('admin.orders.event', $order) }}" method="POST" class="flex items-center gap-2">
                                            @csrf
                                            @method('PATCH')
                                            <input type="hidden" name="event" value="cancel" />
                                            <input type="text" name="cancellation_note" maxlength="500" placeholder="Cancel reason" class="h-[34px] w-36 rounded-md border border-gray-300 px-2 text-xs" />
